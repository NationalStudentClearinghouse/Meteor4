...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="loan-type-totals">
        ...
        <table cellpadding="0" cellspacing="0" class="simple" width="400px">
            ...
            <tbody>
                <xsl:apply-templates select="$awards/AwardType" mode="loan-type-totals">
    ...
    <xsl:template match="AwardType" mode="loan-type-totals">
        <xsl:param name="awards"/>
        <xsl:if test="generate-id() = generate-id($awards/AwardType[current() = .][1])">
        <tr>
            <td><xsl:apply-templates select="."/></td>
            <td><xsl:value-of select="format-number(sum($awards[AwardType = current() and string(number(GrossLoanAmount)) != 'NaN']/GrossLoanAmount) + sum($awards[AwardType = current() and string(number(GrossLoanAmount)) = 'NaN']/AwardAmt), '$###,##0.00')"/></td>
        </tr>
        </xsl:if>
    </xsl:template>
